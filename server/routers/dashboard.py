...
async def get_dashboard_info(
    ...
    return await dashboard_services.get_dashboard_information(
...
async def get_overview(
    ...
    plot_data = await dashboard_services.create_overview_plot_data(
        db=db,
    ...
    project_progress_metrics = await dashboard_services.calculate_project_progress(
        project_id, db
    )
...
async def get_adjudication(
    ...
    def get_markup(base_markup, classification: str) -> list:
        ...
        flat_ontology = project_services.flatten_hierarchical_ontology(
            ontology=ontology
        )
    ...
    try:
        output = {
            ...
            "social": social,
            "entities": {
                **dashboard_services.group_data_by_key(
                    data=entity_markup, key="created_by"
                ),
            },
            **(
                {
                    "relations": {
                        **dashboard_services.group_data_by_key(
                            data=relation_markup, key="created_by"
                        )
                    }
                }
                if project["tasks"]["relation"]
...
async def get_download(
    ...
    flat_ontology = project_services.flatten_hierarchical_ontology(ontology=ontology)
